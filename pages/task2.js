...
export default function Task2() {
    ...
    const getWeatherForRandom10Capitals = (capitalArray) => {
        capitalArray.forEach((capitalItem) => {
            ...
            fetch(
                `https://api.openweathermap.org/data/2.5/weather?q=${capitalItem}&appid=${process.env.NEXT_PUBLIC_WEATHERAPI}`
            )
                .then((res) => {
                        ...
                        <div>
                            ...
                            <ul>
                                {randomWeatherArrCapitals.map((item, index) => {
                                    return (
                                        <li key={item.capitalItem}>
                                            {item.capitalItem} - {item.weather}
                                        </li>
                                    );
                                })}
                            </ul>
